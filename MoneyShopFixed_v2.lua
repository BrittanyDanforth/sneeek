-- Money Shop Server Script - FIXED VERSION v2
-- Place in: ServerScriptService (replace the old MoneyShopFixed with this)
...
local PRODUCT_TO_CASH = {
	...
	[3366420800] = 25000,   -- 25k Cash (fixed from 250000)
}
...
local function updateCashDisplay(player, amount)
	...
	if leaderstats then
		...
		if cashDisplay then
			...
			local showShortCurrency = true
			if Settings and Settings.LeaderboardSettings then
				showShortCurrency = Settings.LeaderboardSettings.ShowShortCurrency
			end
			
			-- Update the display
			if showShortCurrency then
				cashDisplay.Value = convertShort(amount)
			else
				cashDisplay.Value = convertComma(amount)
			end
