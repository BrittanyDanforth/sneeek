--[[
	✨ Clean Purchase Handler - COMPLETE FIXED Version
	...
	- Re-initializes button systems after reset
	- Destroys falling cash parts before reset
--]]
...
local function setupButtonDependency(button)
	...
	if dependency and dependency.Value and dependency.Value ~= "" then
		...
		local function checkDependency()
			-- Check by object name in purchased objects
			for _, obj in ipairs(purchasedObjects:GetChildren()) do
				...
				if depButton then
					...
					if depObjName and depObjName.Value == obj.Name then
						return true
					end
				end
			end
...
print("✅ Purchase Handler COMPLETE FIXED loaded!")
print("🔄 Properly handles all resets and re-initialization")
print("📋 Fixed: Dependency system, hover effects, and cash cleanup")