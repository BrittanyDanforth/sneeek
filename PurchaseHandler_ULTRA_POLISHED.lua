--[[
	✨ ULTRA POLISHED Purchase Handler
	
	Major improvements:
	- Buttons stay perfectly grounded (no floating)
	- Smooth color transitions (no flashing)
	- Proper dependency handling without position glitches
	- Performance optimized with minimal lag
--]]
...
local buttonGroundPositions = {} -- Store the proper ground Y position for each button
...
local function calculateGroundPosition(button)
	...
	if raycast then
		...
		local properY = groundY + (buttonHeight / 2) + 0.05 -- Slightly above ground
...
-- Store original button states AND calculate ground positions
local function storeOriginalButtonStates()
	for _, button in ipairs(buttons:GetChildren()) do
		local head = button:FindFirstChild("Head")
		if head then
			-- Calculate ground position first
	...
	print("📸 Stored original states and ground positions for", #buttons:GetChildren(), "buttons")
end

-- Smooth color transition for button
local function smoothColorTransition(head, targetColor)
	-- Use attribute to track current tween
	local currentTween = head:GetAttribute("ColorTween")
	if currentTween then
		local tween = TweenService:GetPlayingTweens()
		for _, t in ipairs(tween) do
			if t == currentTween then
				t:Cancel()
			end
		end
	end
	...
	head:SetAttribute("ColorTween", tween)
	tween:Play()
	
	-- Clean up attribute after tween completes
	tween.Completed:Connect(function()
		head:SetAttribute("ColorTween", nil)
		head.BrickColor = targetColor
	end)
end

-- Update button colors with smooth transitions
local colorUpdateDebounce = false
local queuedColorUpdate = false

function updateButtonColors(buttonFolder, playerMoney)
	if not currentOwner or not playerMoney then return end
	
	-- Debounce rapid updates
	if colorUpdateDebounce then
		queuedColorUpdate = true
		return
	end
	
	colorUpdateDebounce = true
	...
	for _, button in ipairs(buttonFolder:GetChildren()) do
		...
		if price > 0 then
			local targetColor = playerMoney.Value >= price and BrickColor.new("Lime green") or BrickColor.new("Really red")
	...
	-- Handle debounce
	task.wait(0.1)
	colorUpdateDebounce = false
	
	if queuedColorUpdate then
		queuedColorUpdate = false
		updateButtonColors(buttonFolder, playerMoney)
	end
end
...
-- Setup button dependency system with improved positioning
local function setupButtonDependency(button)
	...
	if dependency and dependency.Value and dependency.Value ~= "" then
		...
		if checkDependency() then
			...
			head.BrickColor = BrickColor.new("Really red") -- Start red
...
for _, button in ipairs(buttons:GetChildren()) do
	task.spawn(function()
		...
		-- Store original CFrame for button press animation
...
print("✅ ULTRA POLISHED Purchase Handler loaded!")
print("🎯 Buttons stay grounded - no floating!")
print("🎨 Smooth color transitions - no flashing!")
print("⚡ Optimized performance - minimal lag!")
...
for _, button in ipairs(buttons:GetChildren()) do
	...
	if dep and dep.Value and dep.Value ~= "" then
		print("  " .. button.Name .. " → waits for: '" .. dep.Value .. "' | spawns: '" .. (obj and obj.Value or "nothing") .. "'")
	else
