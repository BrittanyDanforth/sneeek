--[[
	Updated Purchase Handler (2025 Standards) - ULTRA POLISHED Version
	Handles tycoon purchases and money collection with smooth animations
	Fixed: Part flinging issue, added smooth fade effects, BUTTON POSITIONING
	Added: Maximum juice and satisfying feedback
--]]
...
-- 🎵 ENHANCED SOUND SYSTEM with pitch variation
...
local function fixButtonPositions()
	...
	for _, button in pairs(buttons:GetChildren()) do
		...
		if head and head:IsA("BasePart") then
			...
			elseif head.Position.Y < -10 or head.Position.Y > 100 then
				-- Emergency fix for extremely misplaced buttons
				print("  ⚠️ Button", button.Name, "is way out of position! Moving to Y=5")
				head.CFrame = CFrame.new(head.Position.X, 5, head.Position.Z) * (head.CFrame - head.CFrame.Position)
				fixedCount = fixedCount + 1
			end
		end
	end
...
-- Run position fix after a short delay to ensure everything is loaded
task.wait(1)
fixButtonPositions()
...
-- MODERNIZED PART COLLECTOR WITH SMOOTH FADE AND EFFECTS
	...
	if collector.Name == "PartCollector" then
		...
		collector.Touched:Connect(function(part)
			...
			if cashValue then
				...
				playSound(part, 131961136, true) -- Coin collect sound
...
for _, button in ipairs(buttons:GetChildren()) do
	task.spawn(function()
		...
		if dependency then
			...
			-- 🌈 ENHANCED FADE IN with particles
			if Settings.ButtonsFadeIn then
				...
				-- Pop sound
				playSound(head, 1238528678, true)
...
function processPurchase(button, playerStats)
	...
	if Objects[objectName] then
		...
		if newObject:IsA("Model") and newObject.PrimaryPart then
			...
			if objectName:match("MEGA") or objectName:match("Omega") or objectName:match("Super") or objectName:match("POWER") then
				local light = Instance.new("PointLight")
				light.Brightness = 3
				light.Range = 20
				light.Color = Color3.new(1, 0.8, 0)
	...
	if head then
		...
		if Settings.ButtonsFadeOut then
			...
			task.spawn(function()
				local spiralTime = 0
				while spiralTime < Settings.FadeOutTime do
					spiralTime = spiralTime + task.wait()
					local progress = spiralTime / Settings.FadeOutTime
		...
		playSound(workspace, 421058925, true)
	end
end
...
print("✅ ULTRA POLISHED Purchase Handler loaded successfully! 🎆")
print("✨ Features: Button position fixing, hover effects, particles, sounds, and maximum juice!")