	...
	for _, button in ipairs(buttons:GetChildren()) do
		...
		if head and head:IsA("BasePart") then
			...
			if not fixed then
				...
				warn("⚠️ [Kuromi] Could not find ground for button:", button.Name, "- using default height")
			end
		end
	end

	if fixedCount > 0 or failedCount > 0 then
		print("✅ [Kuromi] Fixed", fixedCount, "button positions,", failedCount, "used fallback")
	end
end
		...
		if checkDependency() then
			...
			for attempt = 1, 3 do
				...
				if raycast then
					...
					fixed = true
					print("📍 [Kuromi] Fixed position for", button.Name, "at Y =", properY)
					break
				else
			...
			if not fixed then
				...
				warn("⚠️ [Kuromi] Could not find ground for", button.Name, "- using default height")
