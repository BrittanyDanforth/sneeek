-- FINAL UNIFIED LEADERBOARD (WITH GATE FIX)
-- Place this ONE script in ServerScriptService
-- FIXED: Now creates PlayerMoney and OwnsTycoon values that gate scripts check for
...
if not playerMoneyFolder then
	...
	print("Created PlayerMoney folder for gate scripts")
end
...
function onPlayerEntered(newPlayer)
	...
	local playerMoney = playerMoneyFolder:FindFirstChild(newPlayer.Name)
	if not playerMoney then
		playerMoney = Instance.new("IntValue")
		playerMoney.Name = newPlayer.Name
		playerMoney.Value = 0
		...
		print("Created PlayerMoney data for", newPlayer.Name)
	end
	...
	if CTF_mode == true then
		...
		if Settings.LeaderboardSettings.ShowCurrency then
			...
			-- Connect to the player money we just created
			task.spawn(function()
...
print("Money API: Ready for MoneyShop")