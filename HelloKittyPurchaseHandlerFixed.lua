--[[
	✨ HelloKitty Purchase Handler - ULTIMATE FIXED Version
	...
	for _, button in ipairs(buttons:GetChildren()) do
		...
		if objectName and objectName.Value then
			...
			if purchaseObject then
				...
				warn("[HelloKitty] Object missing for button:", button.Name, "- Object:", objectName.Value)
			end
		end
	end
...
local function resetTycoonPurchases()
	print("🔄 [HelloKitty] RESETTING TYCOON...")
	...
	print("  ✓ [HelloKitty] Destroyed", destroyedParts, "cash parts")
	...
	print("✅ [HelloKitty] Tycoon fully reset!")
end
...
tycoonOwner.Changed:Connect(function()
	...
	if newOwner == nil and currentOwner ~= nil then
		...
		print("👋 [HelloKitty] Owner left, resetting...")
...
function Purchase(tbl)
	...
	if objectName and objectName.Value and Objects[objectName.Value] then
		...
		print("🎁 [HelloKitty] Spawned:", objectName.Value)
...
print("🔄 [HelloKitty] Dependencies check for spawned objects")
print("📋 [HelloKitty] Properly resets and re-initializes everything")