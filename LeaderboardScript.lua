...
function onPlayerEntered(newPlayer)
	...
	if CTF_mode == true then
		...
		-- Additional stats from settings
		if Settings.LeaderboardSettings.ExtraStats then
			for _, statInfo in pairs(Settings.LeaderboardSettings.ExtraStats) do
				local stat = Instance.new("IntValue")
				stat.Name = statInfo.Name
				stat.Value = statInfo.DefaultValue or 0
				stat.Parent = stats
			end
		end
	...
	-- Connect humanoid events if character already exists
...
function onPlayerRemoving(player)
	-- Clean up any player data if needed
end
...
-- Initialize CTF mode if flag stands exist
