...
local function resetTycoon()
	print("🔄 Resetting HelloKitty tycoon...")
	...
	for _, obj in pairs(script.Parent.PurchasedObjects:GetChildren()) do
		obj:Destroy()
	end
	
	-- Clear collected parts
	collectedParts = {}
	...
	-- Clear BuyObject
	for _, child in pairs(script.Parent.BuyObject:GetChildren()) do
		child:Destroy()
	end
	
	print("✅ HelloKitty tycoon reset complete!")
end

-- Monitor owner changes
script.Parent.Owner.Changed:Connect(function()
	local newOwner = script.Parent.Owner.Value
...
-- Initialize current owner
currentOwner = script.Parent.Owner.Value
...
for i,v in pairs(script.Parent.Essentials:GetChildren()) do
	if v.Name == "PartCollector" then
		v.Touched:connect(function(Part)
			...
			if not currentOwner then return end -- Don't collect without owner
			...
			if Part:FindFirstChild('Cash') then
				...
				-- Clear from tracking after delay
				delay(1, function()
