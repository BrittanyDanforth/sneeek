--[[
	Admin Cash GUI - FINAL WORKING VERSION
...
local createGUIRemote = ReplicatedStorage:FindFirstChild("CreateAdminGUI")
if not createGUIRemote then
	createGUIRemote = Instance.new("RemoteEvent")
	createGUIRemote.Name = "CreateAdminGUI"
	createGUIRemote.Parent = ReplicatedStorage
end
...
Players.PlayerAdded:Connect(function(player)
	...
	if isAdmin(player) then
		print(player.Name .. " is an admin! Sending GUI creation signal...")
		
		-- Tell client to create GUI
		player.CharacterAdded:Connect(function()
			wait(1)
			createGUIRemote:FireClient(player)
		end)
		
		-- Also fire if character already exists
...
-- Create a LocalScript in StarterPlayerScripts to handle GUI creation
local clientScript = Instance.new("LocalScript")
clientScript.Name = "AdminGUIHandler"
clientScript.Parent = game:GetService("StarterPlayer"):WaitForChild("StarterPlayerScripts")

-- This is the content that will run on the client
clientScript.Source = [[
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
-- Wait for remotes
local createGUIRemote = ReplicatedStorage:WaitForChild("CreateAdminGUI")
local giveRemote = ReplicatedStorage:WaitForChild("AdminGiveCash")

-- Function to create GUI
local function createAdminGUI()
	-- Check if GUI already exists
	if player.PlayerGui:FindFirstChild("AdminCashGUI") then
		return
	end
	
	print("Creating Admin GUI for", player.Name)
	...
	giveBtn.MouseButton1Click:Connect(function()
		...
		giveRemote:FireServer(username, amount)
...
-- Listen for GUI creation signal
createGUIRemote.OnClientEvent:Connect(createAdminGUI)
]]
